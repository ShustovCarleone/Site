const translations = {
  en: {
    ...
    'nav.label': 'Main navigation', 'nav.games': 'Games', 'nav.streamers': 'Streamers', 'nav.about': 'About', 'nav.support': 'Support', 'nav.contact': 'Contact', 'nav.cart': 'Cart',
    ...
    'games.eyebrow': 'Available on Steam', 'games.title': 'Choose your kind of chaos.',
    'games.subtitle': 'Two very different games, both built independently with care.', 'games.addCart': 'Add to cart', 'games.viewSteam': 'View on Steam',
    ...
    'gremlins.updateBadge': 'V2.8 · Goblin & Stream Kit',
    'gremlins.tag1': 'Desktop pet', 'gremlins.tag2': 'Simulation',
    'gremlins.description': 'Raise a living desktop gremlin through seven distinct forms. It reacts to your programs and music, chats in its own voice, moves between windows, joins OBS scenes, and lets Twitch viewers interact through Channel Points.',
    'gremlins.feature1': 'Twitch Channel Points, Gremlin Chat, custom replies, and a sequential reward queue', 'gremlins.feature2': '50+ optional desktop effects plus live gremlin statistics for OBS', 'gremlins.feature3': 'Seven character forms and community sprite packs through Steam Workshop',
    ...
    'about.text': 'ShuGhost is an independent developer focused on memorable characters and unusual PC experiences. Each project is made with experimentation at its core — whether that means a creature living above your desktop or a strange night with Dr. Violet.',
    'about.cta': 'See all games', 'about.quote': 'Make something small enough to finish — and strange enough to remember.',
  ...
  uk: {
    ...
    'language.label': 'Мова',
    'nav.label': 'Головна навігація', 'nav.games': 'Ігри', 'nav.streamers': 'Стрімери', 'nav.about': 'Про мене', 'nav.support': 'Підтримка', 'nav.contact': 'Зв’язок', 'nav.cart': 'Кошик',
    ...
    'games.eyebrow': 'Доступно в Steam', 'games.title': 'Оберіть свій вид хаосу.',
    'games.subtitle': 'Дві зовсім різні гри, кожна незалежно створена з турботою.', 'games.addCart': 'Додати в кошик', 'games.viewSteam': 'Переглянути в Steam',
    ...
    'gremlins.description': 'Виростіть живого десктопного гремліна через сім різних форм. Він реагує на програми й музику, говорить у власному стилі, живе у вікнах, з’являється в OBS і дозволяє глядачам Twitch взаємодіяти через бали каналу.',
    'gremlins.feature1': 'Бали каналу Twitch, Gremlin Chat, власні відповіді та послідовна черга нагород', 'gremlins.feature2': 'Понад 50 керованих ефектів робочого столу та жива статистика гремліна для OBS', 'gremlins.feature3': 'Сім форм персонажа й набори спрайтів спільноти у Steam Workshop',
    ...
    'about.text': 'ShuGhost — незалежний розробник, зосереджений на незабутніх персонажах і незвичайних іграх для ПК. В основі кожного проєкту — експеримент: чи то істота, яка живе поверх робочого столу, чи дивна ніч із докторкою Вайолет.',
    'about.cta': 'Переглянути всі ігри', 'about.quote': 'Створюй щось досить невелике, щоб завершити, і досить дивне, щоб запам’ятати.',
  ...
  ru: {
    ...
    'language.label': 'Язык',
    'nav.label': 'Главная навигация', 'nav.games': 'Игры', 'nav.streamers': 'Стримеры', 'nav.about': 'Обо мне', 'nav.support': 'Поддержка', 'nav.contact': 'Связаться', 'nav.cart': 'Корзина',
    ...
    'games.eyebrow': 'Доступно в Steam', 'games.title': 'Выберите свой вид хаоса.',
    'games.subtitle': 'Две совершенно разные игры, каждая независимо создана с заботой.', 'games.addCart': 'Добавить в корзину', 'games.viewSteam': 'Посмотреть в Steam',
    ...
    'gremlins.description': 'Вырастите живого настольного гремлина через семь разных форм. Он реагирует на программы и музыку, говорит в собственном стиле, живёт в окнах, появляется в OBS и позволяет зрителям Twitch взаимодействовать через баллы канала.',
    'gremlins.feature1': 'Баллы канала Twitch, Gremlin Chat, собственные ответы и последовательная очередь наград', 'gremlins.feature2': 'Более 50 управляемых эффектов рабочего стола и живая статистика гремлина для OBS', 'gremlins.feature3': 'Семь форм персонажа и наборы спрайтов сообщества в Steam Workshop',
    ...
    'about.text': 'ShuGhost — независимый разработчик, сосредоточенный на запоминающихся персонажах и необычных играх для ПК. В основе каждого проекта лежит эксперимент: будь то существо, живущее поверх рабочего стола, или странная ночь с доктором Вайолет.',
    'about.cta': 'Посмотреть все игры', 'about.quote': 'Создавай что-то достаточно небольшое, чтобы закончить, и достаточно странное, чтобы запомнить.',
...
if (!supportedLanguages.includes(currentLanguage)) currentLanguage = supportedLanguages.includes(detectedLanguage) ? detectedLanguage : 'en'
...
function renderCart() {
  ...
  document.querySelectorAll('[data-remove-game]').forEach((button) => {
    button.addEventListener('click', () => {
      cart = cart.filter((id) => id !== button.dataset.removeGame)
      saveCart()
      renderCart()
    })
  })
...
function initializeHlsPlayers() {
  document.querySelectorAll('video[data-hls]:not([data-hls-ready])').forEach((video) => {
...
function openCart() {
  ...
  cartDrawer.querySelector('.icon-button')?.focus()
}
...
document.querySelectorAll('[data-add-game]').forEach((button) => {
  button.addEventListener('click', () => {
    const id = button.dataset.addGame
    if (games[id] && !cart.includes(id)) cart.push(id)
    saveCart()
    renderCart()
    openCart()
  })
