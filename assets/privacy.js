(() => {
  ...
  const translations = {
    uk: {
      ...
      counterTitle: "Анонімний лічильник відвідувачів",
      counterText: "Сайт створює випадковий ідентифікатор для поточної вкладки браузера, щоб обчислювати анонімну кількість користувачів онлайн і загальну кількість переглядів. Ідентифікатор зникає після завершення сеансу браузера. Сервер тимчасово використовує мережеву адресу для захисту від повторних переглядів і зловживань, але не записує її у файл статистики.",
    ...
    ru: {
      ...
      counterTitle: "Анонимный счётчик посетителей",
      counterText: "Сайт создаёт случайный идентификатор для текущей вкладки браузера, чтобы подсчитывать анонимное количество пользователей онлайн и общее количество просмотров. Идентификатор исчезает после завершения сеанса браузера. Сервер временно использует сетевой адрес для защиты от повторных просмотров и злоупотреблений, но не записывает его в файл статистики.",
    ...
    en: {
      ...
      counterTitle: "Anonymous visitor counter",
      counterText: "The site creates a random ID for the current browser tab to calculate anonymous online and total view counts. The ID expires with the browser session. The server uses the network address temporarily to prevent duplicate views and abuse, but does not save it in the visitor statistics file.",
